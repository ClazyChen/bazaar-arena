...
- **计数（`Formula.Count(condition)`）**
  ...
  - `Count` **默认不计已摧毁物品**（`it.Destroyed==true` 的候选 item 会被跳过）；物品定义侧通常**不需要**再显式叠 `~Condition.Destroyed` 来“排除摧毁”。
...
- **Trigger.Ammo 与 TargetCondition**：**`Override(trigger: …)` 不再按触发器类型重置 TargetCondition**；**TargetCondition** 保留 **`Ability.xxx` 工厂**已设的值（如 **`Ability.Destroy`** 的 **`DifferentSide`**）。分解射线等 **`Destroy` + `Trigger.Ammo`** 依赖工厂默认即可。
