...
| `scripts/run_item_tests_small_bronze.py` | 小型铜物品自动化测试脚本（用例与断言在此定义） |
...
- 新增物品或新档位测试时：在对应卡组集 JSON 中增加卡组，在相应脚本中增加用例（如小型铜用 `run_item_tests_small_bronze.py`，小型银用 `run_item_tests_small_silver.py`，增加 `name`、`deck1`、`deck2`、`log_contains`），并在 `docs/test-cases-*.md` 与 `docs/items-list.md` 中补充说明与状态列；脚本可扩展为支持多套用例（如中型、大型）或多卡组集。
