...
- **卡组集**：物品测试用卡组定义在 `Data/Decks/test_small_bronze.json`（小型铜）、`Data/Decks/test_small_silver.json`（小型银）、`Data/Decks/test_medium_bronze.json`（中型铜）、`Data/Decks/test_medium_gold_diamond.json`（中型金/钻）、`Data/Decks/test_large_bronze.json`（大型铜）、`Data/Decks/test_large_silver_gold_diamond.json`（大型银/金/钻）等；每个用例对应两个卡组 ID（P1、P2）
...
1. **运行脚本**（仓库根目录）：
   ```bash
   # 小型铜物品
   python scripts/run_item_tests_small_bronze.py

   # 小型银物品
   python scripts/run_item_tests_small_silver.py

   # 中型铜物品
   python scripts/run_item_tests_medium_bronze.py

   # 海盗小型铜物品
   python scripts/run_item_tests_vanessa_small_bronze.py

   # 海盗中型铜物品
   python scripts/run_item_tests_vanessa_medium_bronze.py

   # 大型铜物品
   python scripts/run_item_tests_large_bronze.py

   # 大型银/金/钻物品
   python scripts/run_item_tests_large_silver_gold_diamond.py

   # 中型金/钻物品
   python scripts/run_item_tests_medium_gold_diamond.py
   ```
2. **脚本行为**：
...
| `Data/Decks/test_small_silver.json` | 小型银物品测试用卡组集 |
| `Data/Decks/test_medium_bronze.json` | 中型铜物品测试用卡组集 |
| `Data/Decks/test_medium_gold_diamond.json` | 中型金/钻物品测试用卡组集 |
| `Data/Decks/test_large_bronze.json` | 大型铜物品测试用卡组集 |
| `Data/Decks/test_large_silver_gold_diamond.json` | 大型银/金/钻物品测试用卡组集 |
| `scripts/run_item_tests_small_bronze.py` | 小型铜物品自动化测试脚本 |
| `scripts/run_item_tests_small_silver.py` | 小型银物品自动化测试脚本 |
| `scripts/run_item_tests_medium_bronze.py` | 中型铜物品自动化测试脚本 |
| `scripts/run_item_tests_medium_gold_diamond.py` | 中型金/钻物品自动化测试脚本 |
| `scripts/run_item_tests_large_bronze.py` | 大型铜物品自动化测试脚本 |
| `scripts/run_item_tests_large_silver_gold_diamond.py` | 大型银/金/钻物品自动化测试脚本 |
...
- 新增物品或新档位测试时：在对应卡组集 JSON 中增加卡组，在相应脚本中增加用例（如小型铜用 `run_item_tests_small_bronze.py`，小型银用 `run_item_tests_small_silver.py`，中型铜用 `run_item_tests_medium_bronze.py`，中型金/钻用 `run_item_tests_medium_gold_diamond.py`，大型铜用 `run_item_tests_large_bronze.py`，大型银/金/钻用 `run_item_tests_large_silver_gold_diamond.py`，增加 `name`、`deck1`、`deck2`、`log_contains`），并在 `docs/test-cases-*.md` 与 `docs/items-list.md` 中补充说明与状态列。
