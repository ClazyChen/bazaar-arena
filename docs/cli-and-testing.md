...
- **失败处理**：用例失败时，在 **docs/test-cases/test-cases-small-bronze.md** 与 **docs/items-list.md** 中标注对应物品/用例「**未通过测试**」，并注明失败原因与日志路径，供后续排查与修复；**不通过放宽断言来让测试通过**
...
1. **运行脚本**（仓库根目录）：
   ...
   # 海盗 Vanessa 金/钻批次（大巴扎导入等，卡组集 test_vanessa_gold_diamond_batch.json）
   python scripts/item_tests/run_item_tests_vanessa_gold_diamond_batch.py
   ```
2. **脚本行为**：
...
| `Data/Decks/item_tests/test_vanessa_gold_diamond_batch.json` | 海盗 Vanessa 金/钻批次物品测试卡组集（`vgd_*`） |
...
| `scripts/item_tests/run_item_tests_vanessa_gold_diamond_batch.py` | Vanessa 金/钻批次物品自动化测试脚本 |
| `docs/test-cases/test-cases-small-bronze.md` | 小型铜测试用例说明与预期 |
| `docs/test-cases/test-cases-small-silver.md` | 小型银测试用例说明与预期 |
| `docs/test-cases/test-cases-medium-bronze.md` | 中型铜测试用例说明与预期 |
| `docs/test-cases/test-cases-medium-silver.md` | 中型银测试用例说明与预期 |
| `docs/test-cases/test-cases-medium-gold-diamond.md` | 中型金/钻测试用例说明与预期 |
| `docs/test-cases/test-cases-large-bronze.md` | 大型铜测试用例说明与预期 |
| `docs/test-cases/test-cases-large-silver-gold-diamond.md` | 大型银/金/钻测试用例说明与预期 |
| `docs/test-cases/test-cases-vanessa-gold-diamond-batch.md` | Vanessa 金/钻批次测试用例说明与预期 |
...
## 7. GreedyDeckFinder（与对战 CLI 独立）

- **项目**：`src/BazaarArena.GreedyDeckFinder/BazaarArena.GreedyDeckFinder.csproj`，用于锚定贪心搜索满槽候选，**不是**上文 `BazaarArena.Cli` 的单场对战入口。
- **玩家等级**：`--level` / `playerLevel` 为 **2～20**；池子 `MinTier`、战斗扁平化档位、Overridable 阶梯以 **`GreedyLevelRules`** 为准，与 GUI `Deck.TierAllowedForLevel` 不同。详见 **`docs/greedy-deck-finder.md`**。
- **Python 批跑**：`scripts/run_greedy_vanessa_bronze_top1.py`，说明见 **`docs/scripts-usage.md`** §4。
