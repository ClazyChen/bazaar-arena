...
public interface IBattleLogSink
{
    ...
    void OnCast(ItemState caster, string itemName, int timeMs, int? ammoRemainingAfter = null);
    /// <param name="caster">施放物品。</param>
    /// <param name="extraSuffix">可选后缀，如冻结时为 " →[物品名] →[物品名]"。</param>
    void OnEffect(ItemState caster, string itemName, string effectKind, int value, int timeMs, bool isCrit = false, string? extraSuffix = null);
