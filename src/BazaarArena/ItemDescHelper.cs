...
public static class ItemDescHelper
{
    ...
    /// <summary>描述中可用 {Cooldown} 表示冷却秒数，实际读 CooldownMs；{ChargeSeconds} 表示充能秒数，实际读 Charge（毫秒）。</summary>
    ...
    private static bool IsSecondsKey(string key) => key == "CooldownMs" || key == "Charge";
