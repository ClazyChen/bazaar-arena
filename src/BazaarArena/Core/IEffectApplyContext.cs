...
public interface IEffectApplyContext
{
    ...
    int GetResolvedValue(int key, bool applyCritMultiplier = false, int defaultValue = 0);

    /// <summary>当前能力的目标选择条件（用于冻结/减速/充能/加速/摧毁等多目标效果）；由模拟器从 AbilityDefinition.TargetCondition 注入。</summary>
    Formula? TargetCondition { get; }
    ...
    void AddAttributeToCasterSide(int attributeKey, int value, Formula? targetCondition, int maxTargetCount = 0);
    ...
    void SetAttributeOnCasterSide(int attributeKey, int value, Formula? targetCondition);
    ...
    void ReduceAttributeToSide(int attributeKey, int value, Formula? targetCondition, int maxTargetCount = 0, string? effectLogName = null);

    /// <summary>将本次效果施加报告为触发器原因（如灼烧施加后报告 Trigger.Burn），供模拟器统一 InvokeTrigger；仅部分效果（如 BurnApply）调用。</summary>
    void ReportTriggerCause(int triggerName);
    ...
    void ApplyDestroy(int targetCount, Formula? targetCondition = null);
}
