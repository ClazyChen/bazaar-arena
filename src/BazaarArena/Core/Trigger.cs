...
{
    /// <summary>物品被使用时触发（默认 Condition 为 SameAsCaster）。</summary>
    public const int UseItem = 0;
    /// <summary>其他物品被使用时触发（默认 Condition 为 SameSide & DifferentFromCaster；可重写为 DifferentSide 以监听对方）。</summary>
    public const int UseOtherItem = 1;
    /// <summary>战斗开始时触发（默认 Condition 为 Always）。</summary>
    public const int BattleStart = 2;
