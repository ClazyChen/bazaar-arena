...
public static class Effect
{
    ...
    {
        int value = ctx.GetResolvedValue(Key.Regen, applyCritMultiplier: true);
    ...
    public static Action<IEffectApplyContext> AddAttributeApply(int attributeKey) => ctx =>
    {
        ...
        ctx.AddAttributeToCasterSide(attributeKey, ctx.Value, targetCond, maxTarget);
    };
    ...
    public static Action<IEffectApplyContext> ReduceAttributeApply(int attributeKey) => ctx =>
    {
        var targetCond = ctx.TargetCondition ?? Condition.DifferentSide;
        int countKey = ctx.TargetCountKey ?? Key.ModifyAttributeTargetCount;
        int cap = ctx.GetResolvedValue(countKey, defaultValue: 20);
        int maxTarget = cap >= 20 ? 0 : cap;
        ctx.ReduceAttributeToSide(attributeKey, ctx.Value, targetCond, maxTarget, ctx.EffectLogName);
    };
