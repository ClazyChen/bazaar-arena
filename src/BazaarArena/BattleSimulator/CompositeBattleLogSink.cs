...
public class CompositeBattleLogSink : IBattleLogSink
{
    ...
    public void OnCast(BattleItemState caster, string itemName, int timeMs, int? ammoRemainingAfter = null) { foreach (var s in _sinks) s.OnCast(caster, itemName, timeMs, ammoRemainingAfter); }
    public void OnEffect(BattleItemState caster, string itemName, string effectKind, int value, int timeMs, bool isCrit = false, string? extraSuffix = null) { foreach (var s in _sinks) s.OnEffect(caster, itemName, effectKind, value, timeMs, isCrit, extraSuffix); }
