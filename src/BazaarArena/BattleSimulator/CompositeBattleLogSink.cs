...
public class CompositeBattleLogSink : IBattleLogSink
{
    ...
    public void OnCast(int sideIndex, int itemIndex, string itemName, int timeMs, int? ammoRemainingAfter = null) { foreach (var s in _sinks) s.OnCast(sideIndex, itemIndex, itemName, timeMs, ammoRemainingAfter); }
    public void OnEffect(int sideIndex, int itemIndex, string itemName, string effectKind, int value, int timeMs, bool isCrit = false, string? extraSuffix = null) { foreach (var s in _sinks) s.OnEffect(sideIndex, itemIndex, itemName, effectKind, value, timeMs, isCrit, extraSuffix); }
